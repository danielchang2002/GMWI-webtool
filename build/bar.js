export function plot_bar(ele, data, sample, rank) {
  ...
  if (sample.length == 0) {
    ...
    const data_sum = reduced_data.reduce((prev, curr) => prev + curr.abundance, 0)

    const reduced_data_filled = [...reduced_data, {pop : reduced_data[0].pop, taxon : "Others", abundance : 1 - data_sum}]
    ...
    const sample_sum = reduced_sample.reduce((prev, curr) => prev + curr.abundance, 0)
    const data_sum = reduced_data.reduce((prev, curr) => prev + curr.abundance, 0)

    const reduced_sample_filled = [...reduced_sample, {pop : "Sample", taxon : "Others", abundance : 1 - sample_sum}]
    const reduced_data_filled = [...reduced_data, {pop : reduced_data[0].pop, taxon : "Others", abundance : 1 - data_sum}]
...
function StackedBarChart(data, rank, {
  ...
  svg
    ...
    for (let i = taxons.length - 1; i >= 0; i--) {
      ...
      svg.append("rect").attr("x", x - size).attr("y", y - 0.5 * size).attr("width", size).attr("height", size).style("fill", colors[i])
      .style("stroke", "black")
      .style("stroke-width", 1);
